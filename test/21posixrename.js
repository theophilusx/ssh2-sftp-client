...
const { renameSetup, renameCleanup } = require('./hooks/rename-hooks.js');

chai.use(chaiSubset);
chai.use(chaiAsPromised);

describe('21posixrename: posixRename() method tests', function () {
  let sftp;
  ...
  it('rename should return a promise', function () {
    ...
    return expect(p).to.eventually.equal(`Successful POSIX rename ${from} to ${to}`);
  });
