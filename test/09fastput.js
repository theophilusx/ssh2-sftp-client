...
const { fastPutSetup, fastPutCleanup } = require('./hooks/fast-put-hooks.js');
const { statSync } = require('node:fs');

chai.use(chaiSubset);
chai.use(chaiAsPromised);

describe('09fastput: fastPut() method tests', function () {
  let sftp;
  ...
  it('fastPut rejected when local target a dir', function () {
    return expect(
      sftp.fastPut(makeLocalPath(config.localUrl, 'fp-dir'), `${config.sftpUrl}/fp-dir`),
