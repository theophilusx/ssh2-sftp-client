...
const { chmodSetup, chmodCleanup } = require('./hooks/chmod-hooks.js');

chai.use(chaiSubset);
chai.use(chaiAsPromised);

describe('15chmod: chmod() method tests', function () {
  let sftp;
  ...
  it('chmod should return a promise', function () {
    return expect(sftp.chmod(`${config.sftpUrl}/chmod-test.txt`, 0o444)).to.be.a(
      'promise',
    );
  });
  ...
  it('chmod on a file reports correct mode', function () {
    ...
    return sftp
      ...
      .then((list) => {
        return expect(list.filter((e) => (e.name = 'chmod-test.txt'))).to.containSubset(
          results,
        );
      });
  });
  ...
  it('chmod on relative file path 2', function () {
    ...
    let remotePath = `../${lastRemoteDir(config.remoteRoot)}/testServer/chmod-test.txt`;
    return sftp
  ...
  it('chmod on relative path dir 4', function () {
    ...
    let remotePath = `../${lastRemoteDir(config.remoteRoot)}/testServer/chmod-test-dir`;
    return sftp
