...
prior to v18.x are not supported. 
...
us8ing promises is challenging . To understand some of the more subtle issues involved, it
...
The previous versions of this package used a global event handler approach to manage
events raised outside the execution of any promise being evaluated by methods in this
package. However, this approach is problematic as all the global listeners can really do
is raise an error and attempting to catch errors raised inside event handlers is extremely
difficult to manage within client code. In version 11, this approach has been replaced by
a mechanism whereby the client code can pass in application specific global handlers if
desired. If no handlers are defined, default handlers will log the event and when
necessary, invalidate any existing connection objects. See the [[Background]] section for
details. 
...
asynchrounous code execution. This package uses the following strategies;
    ...
    - All direct interactions with the ~ssh2~ API are wrapped in a promise object. When the
      API call succeeds, the promise will be sucessfully resulved. When an error occurs,
      the promise is rejected.

    - An error can be either due to a low level network error, such as a lost connection
      to the remote server or due to an operational error, such as a file not
      existing or not having the appropriate permissions for acdcess.

    - Low level errors and network events, such as ending or closing of a socket
      connection are communicated from the ~ssh~ package vi8a events. The ~ssh2-sftp-client~
      package creates two groups of event handlers to respond to events emitted by the
      ~ssh2~ package;

          1. Temporary promise specific listeners for end, close and error events which
             are created when a new promise object is defined and which are tied to the
             promise via its reject method. If an error, end or close event is raised
             while the promise is executing, the promise will be rejected. These listeners
             are removed once the promise is fulfilled.

          2. Global event listeners for end, close and error events. These listeners are
             created when the ~SftpClient~ object is instantiated and will only be executed
             if an end, close or error event is raised and is not handled by a
             temporary promise listener. By default, the global listener will just log the
             event and invalidate the connection object, preventing any further
             operations until a new connection is established. It is also possible to set
             applicatioln specific listeners via the ~SftpClient~ constructor.
...
API call, a promise is active and can be used to communicate events back to the calling
code via normal promise communication means i.e. async/await with try/catch or promise chain's then/catch
mechanism. However, outside API calls, no promise exists and there is no standard reliable
...
calling code, ~ssh2-sftp-client~ doesn't try to by default. Instead, the default action is
to just log the event information and invbalidate any existing sftp connections This
